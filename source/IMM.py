...
class IMM(object):
    def __init__(self, input, max_components=20, b=1e6, c=1e-5, lambda0=5):
        ...
        kmeans = KMeans(init='k-means++', n_clusters=self.m, n_init=1)
    ...
    ###
        ...
        self.temp = var_c_norm

    ###      
        
        self.MuS = np.reshape(kmeans.cluster_centers_, (self.m,))
        self.mS = self.MuS
        self.vS = np.asarray([var_c_norm]).flatten()
    ...
    def Reset(self, input):
        ...
        self.MuS = np.reshape(kmeans.cluster_centers_, (self.m,))
        self.mS = self.MuS
        self.vS = np.asarray([(kmeans.inertia_)**2 / np.float(self.N)] * self.m)
    ...
    def Mstep(self):
        ...
        vS_ = self.vS#(1/bS_) / (self.N*pi_s_mean)
